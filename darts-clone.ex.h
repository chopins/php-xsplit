...
private:
	...
	void expand_double_array()
	{
		...
		links_.resize(dest_num_of_units);
...
private:
	...
	// Accesses a link.
	link_type &links(base_type index) { return links_[index]; }
...
public:
	...
	// Finding keys starting with a specified prefix.
	class Finder
	{
	private:
		const DoubleArrayBase *da_;
		std::vector<char_type> key_;
		value_type value_;
		std::stack<base_type> index_stack_;

	public:
		Finder(const DoubleArrayBase &da, const char_type *prefix,
			size_type length)
			: da_(&da), key_(length + 1), value_(0), index_stack_()
		{
		...
		Finder(const DoubleArrayBase &da, const char_type *prefix,
			size_type length, base_type index)
			: da_(&da), key_(length + 1), value_(0), index_stack_()
		{
			for (size_type i = 0; i < length; ++i)
				key_[i] = prefix[i];
		...
		// Finds the next key starting with a given prefix.
		bool next()
		{
			...
			// Backtracks.
			if (da_->units_[index].is_leaf())
			{
				while (!da_->links_[index].bros())
				{
					if (index_stack_.empty())
						return false;
					index = pop();
				}
				index ^= da_->links_[index].bros();
				push(index, da_->units_[index].label());
			}

			// Repeats transitions until finds a leaf.
			do
			{
				uchar_type child_label = da_->links_[index].child();
				index ^= da_->units_[index].offset() ^ child_label;
				push(index, child_label);
			} while (!da_->units_[index].is_leaf());
			value_ = da_->units_[index].value();

			return true;
		}

		// Return key information.
		const char_type *c_str() const { return &key_[0]; }
		size_type length() const { return key_.size() - 1; }
		size_type size() const { return length(); }
		result_type value() const { return static_cast<result_type>(value_); }

	private:
		base_type pop()
		{
			base_type index = index_stack_.top();
			index_stack_.pop();
			key_.resize(key_.size() - 1);
			return index;
		}
		void push(base_type index, base_type label)
		{
			index_stack_.push(index);
			key_.push_back(static_cast<char_type>(label));
		}
	};

private:
	const unit_type *units_;
	const link_type *links_;
	size_type size_;
	std::vector<unit_type> units_buf_;
	std::vector<link_type> links_buf_;
...
public:
	DoubleArrayBase() : units_(0), links_(0), size_(0),
		units_buf_(), links_buf_() {}
		...
		int (*progress_func)(size_type, size_type) = 0)
	{
		...
		replace_links_buf(builder.links_buf());
		return 0;
	}
	...
	{ return sizeof(unit_type) + sizeof(link_type); }
	// Always returns the number of units (no use).
	size_type nonzero_size() const { return size(); }
	// Returns the array size.
    size_type total_size() const { return size_ * unit_size(); }

	// Sets the start address of an array.
	// Warning: the first integer is used as the size.
	void set_array(const void *ptr, size_type size = 0)
	{
		clear();

		size_ = static_cast<size_type>(*static_cast<const base_type *>(ptr));

		units_ = static_cast<const unit_type *>(ptr) + 1;
		links_ = reinterpret_cast<const link_type *>(units_ + size_);

		LOG_VALUE(total_size());
	}
	// Returns the start address of an array.
	// Warning: this method is obsolete, must not be used.
	...
	void clear()
	{
		...
		std::vector<link_type>(0).swap(links_buf_);
	}
		...
		size_type offset = 0, size_type size = 0)
	{
		...
		size_type num_of_units = size / unit_size();

		base_type stored_size;
		if (!file.read(&stored_size, 1) || stored_size != num_of_units)
			return -1;

		std::vector<unit_type> new_units_buf(num_of_units);
		std::vector<link_type> new_links_buf(num_of_units);
		if (!file.read(&new_units_buf[0], num_of_units) ||
			!file.read(&new_links_buf[0], num_of_units))
			return -1;

		clear();
		replace_units_buf(new_units_buf);
		replace_links_buf(new_links_buf);
		return 0;
	}
		...
		size_type offset = 0) const
	{
		...
		base_type size = static_cast<base_type>(size_);
		if (!file.write(&size, 1) || !file.write(units_, size_)
			|| !file.write(links_, size_))
			return -1;
		return 0;
	}
...
public:
	...
	// Searches keys starting with a given prefix.
	Finder predictiveSearch(const char_type *prefix,
		size_type length = 0, size_type node_pos = 0) const
	{
		if (!length)
			length = length_of(prefix);

		// Transitions.
		base_type index = static_cast<base_type>(node_pos);
		unit_type unit = units_[index];
		size_type prefix_pos = 0;
		while (prefix_pos < length)
		{
			index ^= unit.offset_if()
				^ static_cast<uchar_type>(prefix[prefix_pos]);
			unit = units_[index];
			if (unit.label() != static_cast<uchar_type>(prefix[prefix_pos]))
				return Finder(*this, prefix, length);
			++prefix_pos;
		}

		return Finder(*this, prefix, length, index);
	}
...
private:
	...
	void replace_units_buf(std::vector<unit_type> &new_units_buf)
	{
		...
		size_ = new_units_buf.size();
		units_ = &new_units_buf[0];
		units_buf_.swap(new_units_buf);
	}

	// Replaces a current vector with a given vector.
	void replace_links_buf(std::vector<link_type> &new_links_buf)
	{
		if (new_links_buf.empty())
			THROW("empty link vector");

		links_ = static_cast<const link_type *>(&new_links_buf[0]);
		links_buf_.swap(new_links_buf);
	}
